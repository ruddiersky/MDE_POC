    ...
    with open(path, 'r', encoding='utf-8', errors='ignore') as f:
        for line in f:
            if 'ServiceANR' in line:
                ...
                events.append({'timestamp': ts, 'package': pkg, 'line': line.strip()})
            elif 'exitType: ANR' in line:
                m = re.match(r'(\d{2}-\d{2} \d{2}:\d{2}:\d{2}\.\d+)', line)
                ts = m.group(1) if m else ''
                events.append({'timestamp': ts, 'package': '', 'line': line.strip()})
    return events
...
def main():
    ...
    for i, e in enumerate(fc_events, 1):
        print(f"[{i}] Time: {e['timestamp']} Package: {e['package']}")
    ...
    for i, e in enumerate(anr_events, 1):
        ...
        print(f"[{i}] Time: {e['timestamp']}{pkg}")
